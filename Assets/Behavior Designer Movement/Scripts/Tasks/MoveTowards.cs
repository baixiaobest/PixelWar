...
namespace BehaviorDesigner.Runtime.Tasks.Movement
{
    ...
    public class MoveTowards : Action
    {
		...
		[Tooltip("Only look at target horizontally")]
		public SharedBool lookHorizontally;
        ...
        public override TaskStatus OnUpdate()
        {
            ...
            if (lookAtTarget.Value && (position - transform.position).sqrMagnitude > 0.01f) {
				if (!lookHorizontally.Value) {
					...
					Vector3 lookVector = new Vector3 (relativePos.x, 0, relativePos.z);
					transform.rotation = Quaternion.RotateTowards(transform.rotation, Quaternion.LookRotation(lookVector), maxLookAtRotationDelta.Value);
				}
            }
